...
surveyor_df = surveyor_df.sample(n=2000, random_state=42)
garage_df = garage_df.sample(n=2000, random_state=42)
...
# Evaluation Metrics
# Assuming ground-truth labels are provided in the dataset as 'True_Label'
anomaly_data["True_Label"] = np.where(anomaly_data["TOTAL_AMOUNT_y"] > 5000, -1, 1)  # Example condition for labeling
true_labels = anomaly_data["True_Label"].values
predicted_labels = anomaly_data["Anomaly_Score"].values
